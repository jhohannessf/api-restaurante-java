...
import br.com.jhohannesfreitas.restaurante.dto.CozinhaItemResponse;
...
public class CozinhaService {
    ...
    public CozinhaItemResponse entregarItem(Long itemId) {
        ...
        if (item.getStatus() != StatusItemPedido.PRONTO) {
            throw new RegraNegocioException("Somente itens prontos podem ser marcados como pronto.");
        }
    ...
    private PedidoItem buscarItemPorId(Long itemId) {
        return pedidoItemRepository.findById(itemId).orElseThrow(() -> new RegraNegocioException("Item do pedido não encontrado"));
    }
}
